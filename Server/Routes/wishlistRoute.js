...
router.get("/get", authMiddleware, async (req, res) => {
  try {
    ...
    const wishlistItems = await WishlistItem.find({ user: userId }).populate(
      "product",
    ); // Populate the 'product' field to get product details
...
router.delete("/delete/:_itemId", authMiddleware, async (req, res) => {
  try {
    ...
    const deletedItem = await WishlistItem.findOneAndRemove({
      product: _itemId,
      user: userId,
    });
    ...
    res.json({ message: "Item removed from wishlist.", wishlistItems });
  } catch (err) {
    res.status(500).json({ error: "Internal Server Error" });
  }
});
