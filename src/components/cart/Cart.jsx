    ...
    <>
      <div className=" h-[100vh] bg-white z-50 border-l-2 overflow-x-hidden">
        ...
        <div className="flex w-52 my-2 ml-4 "> 
          ...
          <Link to="/checkout">
            <button disabled= {productTPrice==0?true:false}  className={`flex ml-auto text-white border-0 py-2 px-6 focus:outline-none  rounded ${productTPrice==0?'bg-purple-200':' bg-purple-500 hover:bg-purple-600'}`}>
              Checkout
            </button>
          </Link>
          <button disabled= {productTPrice==0?true:false} className={`flex ml-auto text-white border-0 py-2 px-6 focus:outline-none rounded ${productTPrice==0?'bg-purple-200':' bg-purple-500 hover:bg-purple-600'}`}  
